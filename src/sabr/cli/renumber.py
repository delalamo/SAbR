...
def _copy_structure_with_chain_transform(
    ...
    """Create a new structure by copying chains and transforming one.

    This is a helper function that consolidates the common pattern of
    creating a new structure where non-target chains are copied as-is
    and the target chain is transformed.

    Args:
        structure: BioPython Structure object.
        chain_id: Chain identifier to transform.
        structure_name: Name for the new structure.
        transform_chain: Callable that takes a chain and returns a new chain.

    Returns:
        New structure with transformed target chain.
    """
    ...
    for chain in structure[0]:
        if chain.id != chain_id:
            ...
            new_chain = transform_chain(chain)
...
def _thread_structure(
    ...
    return _copy_structure_with_chain_transform(
        structure, chain_id, "renumbered_structure", renumber_chain
    )
...
def _extract_chain_subset(
    ...
    return _copy_structure_with_chain_transform(
        structure, chain_id, "subset_structure", extract_subset
    )
