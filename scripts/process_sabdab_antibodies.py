...
This script downloads antibody structures from SAbDab, organizes them by
species and chain type, classifies CDR-H3 conformations, clusters sequences,
and computes MPNN embeddings for structural analysis.
...
Step-by-step breakdown:
  ...
  3. Organize by species/chain     10-30 minutes
         Single-threaded, 2.4 GB output
...
class PipelineState:
    ...
    def add_failed_file(self, step: str, file_path: str, error: str):
        ...
        self.state["failed_files"][step].append(
            {"file": file_path, "error": str(error)}
        )
        self.save()
...
def download_summary(output_dir: Path, state: PipelineState) -> Path:
    ...
    try:
        ...
        LOGGER.info(
            f"✓ Summary downloaded: {summary_file} ({file_size:,} bytes)"
        )
...
def download_imgt_structure(pdb_id: str, output_dir: Path) -> bool:
    ...
    url = (
        "https://opig.stats.ox.ac.uk/webapps/sabdab-sabpred/"
        f"sabdab/structurefile/{pdb_id}/imgt"
    )
...
def download_structures(
    summary_file: Path, output_dir: Path, state: PipelineState
) -> Path:
    """
    ...
    if existing > 0:
        LOGGER.info(
            f"Found {existing} existing structures, resuming download..."
        )
        downloaded = existing
    ...
    for pdb_id in tqdm(pdb_ids, desc="Downloading structures"):
        try:
            if download_imgt_structure(pdb_id, structures_dir):
                ...
                state.add_failed_file(
                    "download_structures", pdb_id, "Download failed"
                )
...
def normalize_species(species_str: str) -> str:
    ...
    if "homo sapiens" in species_lower or "human" in species_lower:
        ...
        "lama" in species_lower
        or "alpaca" in species_lower
        or "vicugna" in species_lower
    ):
        return "llama_alpaca"
    else:
        return "other"
...
def extract_chain_from_pdb(
    pdb_path: Path, chain_id: str, output_path: Path
) -> bool:
...
def organize_antibodies(
            ...
            for row in tqdm(reader, desc="Organizing structures"):
                try:
                    ...
                    if not pdb_file.exists():
                        skipped += 1
                        state.add_failed_file(
                            "organize", str(pdb_file), "File not found"
                        )
                        continue
                    ...
                    if h_chain and h_chain != "NA":
                        try:
                            ...
                            if extract_chain_from_pdb(
                                pdb_file, h_chain, output_file
                            ):
                                stats[species]["heavy"] += 1
                                processed += 1
                        except Exception as e:
                            state.add_failed_file(
                                "organize", f"{pdb}_{h_chain}", str(e)
                            )
                    ...
                    if l_chain and l_chain != "NA":
                        try:
                            ...
                            if extract_chain_from_pdb(
                                pdb_file, l_chain, output_file
                            ):
                                stats[species]["light"] += 1
                                processed += 1
                        except Exception as e:
                            state.add_failed_file(
                                "organize", f"{pdb}_{l_chain}", str(e)
                            )
    ...
    for species in ["human", "mouse", "llama_alpaca", "other"]:
        if species in stats:
            heavy = stats[species]["heavy"]
            light = stats[species]["light"]
            LOGGER.info(f"  {species}: H={heavy}, L={light}")
...
def get_ca_coordinates(
    pdb_file: Path, residue_numbers: List[int]
...
def classify_cdrh3(tau: float, alpha: float) -> str:
    ...
    distance = ((tau - tau_center) / tau_threshold) ** 2 + (
        (alpha - alpha_center) / alpha_threshold
    ) ** 2

    return "kinked" if distance <= 1.0 else "extended"


def analyze_heavy_chain(
    pdb_file: Path,
    ...
    try:
        ...
        alpha = calculate_dihedral(
            coords[115], coords[116], coords[117], coords[118]
        )
...
def classify_cdrh3_loops(organized_dir: Path, state: PipelineState) -> Path:
    ...
    for species in ["human", "mouse", "llama_alpaca", "other"]:
        ...
        has_kinked = (heavy_dir / "kinked").exists()
        has_extended = (heavy_dir / "extended").exists()
        if has_kinked or has_extended:
            LOGGER.info("%s heavy chains already classified", species)
            continue
        ...
        LOGGER.info(
            f"Classifying {species} heavy chains: {len(pdb_files)} files"
        )
        ...
        for pdb_file in tqdm(pdb_files, desc=f"{species} heavy"):
            try:
                ...
                if classification is None:
                    ...
                    state.add_failed_file(
                        "classify", str(pdb_file), "Missing residues"
                    )
                else:
    ...
    total_failed = sum(
        stats[sp]["unclassified"] + stats[sp]["failed"] for sp in stats
    )
...
def extract_sequence_from_pdb(pdb_file: Path) -> Optional[str]:
    ...
    try:
        ...
        LOGGER.debug("Failed to parse %s: %s", pdb_file, exc)
        return None
...
def cluster_with_mmseqs(
    fasta_file: Path, output_prefix: Path, min_seq_id: float = 0.7
) -> Optional[Path]:
...
def select_best_representatives(
    clusters: Dict,
    category_dir: Path,
    ...
    for rep, members in clusters.items():
        ...
        if isinstance(best_resolution, str) or best_resolution == float("inf"):
            resolution_value = "NA"
        else:
            resolution_value = best_resolution
...
def cluster_sequences(
    ...
    for species, chain_type, classification in categories:
        ...
        try:
            ...
            cluster_tsv = cluster_with_mmseqs(
                fasta_file, output_prefix, min_seq_id
            )

            if cluster_tsv is None:
                LOGGER.error(f"Clustering failed for {category_name}")
                state.add_failed_file(
                    "cluster", category_name, "MMSeqs2 failed"
                )
                continue
...
def save_embeddings_npz(embeddings_dict: Dict, output_file: Path):
    ...
    for struct_id, embeddings in embeddings_dict.items():
        ...
        save_dict[f"{struct_id}_pdb_indices"] = np.array(
            embeddings.idxs, dtype=object
        )

    np.savez_compressed(output_file, **save_dict)


def compute_embeddings_for_category(
    category_name: str,
    pdb_files: List[str],
    output_dir: Path,
    transformed_embed_fn,
    model_params: Dict,
    key,
    state: PipelineState,
    ...
    for pdb_path in tqdm(pdb_files, desc=f"Computing {category_name}"):
        try:
            ...
            if not pdb_path.exists():
                ...
                state.add_failed_file(
                    "embeddings", str(pdb_path), "File not found"
                )
                continue
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Process SAbDab antibody structures and compute MPNN embeddings"
        ),
    ...
    parser.add_argument(
        "--skip-clustering", action="store_true", help="Skip clustering step"
    )
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(e.g., human_heavy_kinked,mouse_light_all)"
        ),
    )
...
def main():
    ...
    try:
        ...
        if args.skip_download:
            ...
            structures_dir = download_structures(
                summary_file, output_dir, state
            )

        # Step 3: Organize antibodies
        organized_dir = organize_antibodies(
            summary_file, structures_dir, output_dir, state
        )
        ...
        LOGGER.info("    summary.tsv")
        LOGGER.info("    all_structures/imgt/  (~8.2 GB)")
        LOGGER.info("    organized_antibodies/  (~2.4 GB)")
        LOGGER.info("      cluster_representatives.csv")
        LOGGER.info("    mpnn_embeddings/  (~3-7 GB)")
