...
from sabr.embeddings import backend as embeddings_backend
...
class TestCreateGapPenaltyForReducedReference:
    ...
    def test_returns_correct_shapes(self):
        ...
        gap_extend, gap_open = (
            embeddings_backend.create_gap_penalty_for_reduced_reference(
                query_len, idxs
            )
        )
    ...
    def test_zero_penalties_at_multiple_boundaries(self):
        ...
        query_len = 50
        # Simulate reduced ref with CDR1, CDR2, pos 10, DE loop, CDR3 removed
        ...
        for col in range(len(idxs)):
            if col not in jumps and col > 0:  # Skip first column
                ...
                assert np.allclose(
                    gap_extend[:, col], expected_extend
                ), f"Gap extend at col {col}"
                assert np.allclose(
                    gap_open[:, col], expected_open
                ), f"Gap open at col {col}"
...
class TestReducedEmbeddings:
    ...
    def test_embeddings_no_cdr_excludes_variable_positions(self):
        ...
        for pos in variable_positions:
            assert (
                pos not in idxs
            ), f"Variable position {pos} should not be in reduced embeddings"
    ...
    def test_embeddings_no_cdr_has_framework_positions(self):
        ...
        for pos in framework_positions:
            assert (
                pos in idxs
            ), f"Framework position {pos} should be in reduced embeddings"
    ...
    def test_embeddings_no_cdr_shape_consistency(self):
        ...
        assert (
            stdev.shape == array.shape
        ), "Stdev shape should match array shape"
...
class TestSoftAlignerWithReducedEmbeddings:
    ...
    def test_softaligner_loads_both_embeddings(self):
        ...
        assert (
            n_reduced < n_full
        ), "Reduced embeddings should have fewer positions"
    ...
    def test_softaligner_uses_reduced_when_deterministic(self):
        ...
        # The reduced embeddings should be loaded
        ...
        for pos in cdr_positions:
            assert (
                pos not in reduced_idxs
            ), f"CDR position {pos} should not be in reduced embeddings"
